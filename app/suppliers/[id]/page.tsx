...
export default function SupplierDetailPage() {
    ...
    const [activeTab, setActiveTab] = useState<'orders' | 'payments' | 'analytics'>('orders');
            ...
            <div className="print-area h-full w-full">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex gap-3 no-print">
                        <button
                            onClick={handlePrint}
                            className="bg-white border border-gray-200 text-gray-700 px-6 py-3 rounded-2xl font-black flex items-center gap-2 hover:bg-gray-50 transition-all shadow-sm"
                        >
                            <Printer size={20} /> تصدير كشف الحساب
                        </button>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="xl:col-span-2 flex flex-col gap-6">
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-2xl w-fit no-print">
                            ...
                            <button onClick={() => setActiveTab('payments')} className={`px-5 py-2.5 rounded-xl text-sm font-black transition-all ${activeTab === 'payments' ? 'bg-white text-green-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>سجل المدفوعات</button>
                        ...
                        <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden mb-8">
                                ...
                                <div className="no-print">
                                    <div className="p-6 border-b border-gray-50">
                                        <h3 className="text-lg font-black text-gray-900 flex items-center gap-2">
                                            <CreditCard size={18} className="text-green-600" /> كشف حساب المدفوعات
                                        </h3>
                                    </div>
                                    {!supplier.payments || supplier.payments.length === 0 ? (
                                        <div className="py-16 text-center text-gray-300 flex flex-col items-center gap-3">
                                            <CreditCard size={48} />
                                            <p className="font-black">لا توجد سجلات دفع لهذا المورد</p>
                                        </div>
                                    ) : (
                                            ...
                                            <table className="w-full text-right text-sm">
                                                <thead className="bg-gray-50 border-b border-gray-200 text-gray-600">
                                                    <tr>
                                                        ...
                                                        <th className="px-5 py-4 font-semibold">المبلغ المُسدد (دج)</th>
                                                        <th className="px-5 py-4 font-semibold">ملاحظات</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="divide-y divide-gray-200">
                                                    {supplier.payments.map(payment => (
                                                        <tr key={payment.id} className="hover:bg-gray-50/50 transition-colors">
                                                            <td className="px-5 py-4 font-bold text-gray-900">{formatDate(payment.paymentDate)}</td>
                                                            <td className="px-5 py-4 font-black text-green-600 font-sans" dir="ltr">{payment.amount.toLocaleString()}</td>
                                                            <td className="px-5 py-4 text-gray-500">{payment.notes || '-'}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                            ...
                            {activeTab === 'orders' && (
                                <div className="no-print">
                                    <div className="p-6 border-b border-gray-50">
                                        <h3 className="text-lg font-black text-gray-900 flex items-center gap-2">
                                            <ShoppingCart size={18} className="text-indigo-600" /> سجل الطلبيات
                                        </h3>
                                    </div>
